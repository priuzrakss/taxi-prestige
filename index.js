...
app.post('/api/order', orderLimiter, (req, res) => {
    ...
    if (!orderData.fromAddress || !orderData.toAddress || !orderData.date || !orderData.time || !orderData.name || !orderData.phone || !orderData.price || !orderData.distance) {
        console.error('Ошибка: Одно из обязательных полей отсутствует.');
        return res.status(400).json({ message: 'Одно из обязательных полей отсутствует. Проверьте данные и попробуйте снова.' });
    }
...
- *Километраж*: ${orderData.distance || 'Не указано'} км
    `;
